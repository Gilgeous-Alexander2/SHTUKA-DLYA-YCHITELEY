...
function Tablichki(props) {
  React.useEffect(() => {
    let Sostnum = JSON.parse(localStorage.getItem("SostNomer"));
    if (Sostnum) { setNomerklassa(Sostnum) }
    else {
      Sostnum = [1, 2, 3, 4, 5, 6, 7]
      setNomerklassa(Sostnum);
      localStorage.setItem("SostNomer", JSON.stringify(Sostnum));
    }
    ...
    <div style={{ display: 'flex', gap: '50px', paddingLeft: '150px', paddingRight: '100px', flexWrap: 'wrap', height: '55px' }}>
      ...
      {props.knop.map(item => <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
        <NavLink to={`/items${props.tab}/${item.id}`}><CButton style={{ minWidth: '60px', height: '60px' }} name="knopkaaa" id={`knopkaaa${item.dl}`} key={item.id} onClick={() => { props.setCount(item.dl); setColor(item.dl) }}>
          {props.tab == 1 && props.userData.name === "admin" ? <input style={{ width: '20px' }} value={nomerklassa[item.dl]} onChange={(e) => {
            setNomerklassa(
              old => {
                const newState = [...old];
      ...
      {props.userData.name === "admin" && props.tab == 1 ? <>
        ...
        {props.knop.length <= 6 && <CButton onClick={() => {
          props.setKnop(old => [...old, { id: uuidv4(), name: `${nomerklassa[props.knop.length]}`, dl: props.knop.length }]);
        }
        }><AddIcon fontSize='large' /></CButton>}{localStorage.setItem("Sostknop", JSON.stringify(props.knop))}</> : null}
